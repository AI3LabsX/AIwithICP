...
def get_data_sep(sources: Path | str, embeddings: OpenAIEmbeddings) -> None:
    for file in os.listdir(sources):
        logger.info(f"Processing file {file}")
        if file.lower().endswith('.pdf'):
            try:
                pdf_path = Path(sources).joinpath(str(file))
                loader = PyMuPDFLoader(str(pdf_path))
                documents = loader.load()  # Load documents from the current file

                # Process the documents and create a vector store for the current file
                db = process_recursive(documents, embeddings)
                # index_name = file.replace('.pdf', '')  # Create index name based on the filename
                db.save_local(str(file))  # Save the vector store with the index name
...
if __name__ == '__main__':
    data = PROJECT_ROOT.joinpath('READ')
    embeddings = OpenAIEmbeddings()
    ids = str(uuid.uuid4())
    # get_data(data, embeddings, f'faiss_index_ICP_READ')
    get_data_sep(data, embeddings)
